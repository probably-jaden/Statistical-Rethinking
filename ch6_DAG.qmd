...
ggplot(data, aes(x = x, y = y)) +
  ...
  geom_smooth(data = . %>% filter(above_line), method = "lm", se = FALSE, color = "blue4") +
...
as_tibble(b6.1) %>% 
  rename("Percent Fat" = b_fat_z) %>% 
  dplyr::select(c(`Percent Fat`, `Intercept`)) %>% 
  pivot_longer(cols = everything(),
               names_to = "Covariate",
               values_to = "Effect") %>% 
...
as_tibble(b6.1b) %>% 
  rename("Percent Lactose" = b_lactose_z) %>% 
  dplyr::select(c(`Percent Lactose`, `Intercept`)) %>% 
  pivot_longer(cols = everything(),
               names_to = "Covariate",
               values_to = "Effect") %>% 
  ggplot(aes(x = `Effect`, y = reorder(Covariate, `Effect`))) +
...
as_tibble(b6.1c) %>% 
  rename("Fat Percent" = b_fat_z,
         "Lactose Percent" = b_lactose_z) %>% 
  dplyr::select(c(`Fat Percent`, `Lactose Percent`, `Intercept`)) %>% 
  pivot_longer(cols = everything(),
               names_to = "Covariate",
               values_to = "Effect") %>% 
  ggplot(aes(x = `Effect`, y = reorder(Covariate, `Effect`))) +
...
as_tibble(b6.2) %>% 
  rename("Intercept" = b_p_Intercept) %>% 
...
as_tibble(b6.3) %>% 
  rename("Intercept" = b_p_Intercept,
         "Treatment" = b_p_treatment, 
         "Fungus" = b_p_fungus) %>% 
  dplyr::select(c(`Treatment`, `Fungus`, `Intercept`)) %>% 
  pivot_longer(cols = everything(),
               names_to = "Covariate",
               values_to = "Effect") %>% 
  ggplot(aes(x = `Effect`, y = reorder(Covariate, `Effect`))) +
  stat_halfeye(point_interval = median_qi, .width = .95,
               fill = "firebrick4") +
  labs(x = "Percent Growth Effect on Plant",
       y = NULL) +
  theme_bw() +
  theme(axis.text.y = element_text(hjust = 0),
        axis.ticks.y = element_blank(),
...
as_tibble(b6.4) %>% 
  rename("Intercept" = b_p_Intercept,
         "Treatment" = b_p_treatment) %>% 
  dplyr::select(c(`Treatment`, `Intercept`)) %>% 
  pivot_longer(cols = everything(),
               names_to = "Covariate",
               values_to = "Effect") %>% 
  ggplot(aes(x = `Effect`, y = reorder(Covariate, `Effect`))) +
...
b6.8 <- brm(data = familySim,
            ...
            iter = 2000, warmup = 500, cores = 4, seed = 4,
            backend = "cmdstanr", silent = 2, file = "fits/b06.8.1"
            )
...
as_tibble(b6.8) %>% 
  rename("Grandparents" = b_G,
         "Parents" = b_P,
         "Unobserved" = b_U) %>% 
  dplyr::select(c(`Grandparents`, `Parents`, `Unobserved`, `Intercept`)) %>% 
  pivot_longer(cols = everything(),
               names_to = "Covariate",
               values_to = "Effect") %>% 
  ggplot(aes(x = `Effect`, y = reorder(Covariate, `Effect`))) +
...
m7 <- dagify("W" ~ "E" + "U",
  ...
  theme_bw() +
  theme(panel.grid = element_blank())+
  labs(caption = "W = Wages, E = Education, U = Unobserved")
...
dag_coords <- tibble(name = c("X", "Z", "Y"),
                     x = c(1, 2, 3),
                     y = c(2, 1, 2))
...
dag_coords <- tibble(name = c("X", "Z", "D", "Y"),
                     x = c(1, 2, 2, 3),
                     y = c(1, 2, 1.3, 1))
